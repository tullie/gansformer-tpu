...
def all_sum(cores, g, colocate=True, *args, **kws):
  ...
  #   return all_sum_gpu(g, *args, **kws)
     ...
     assert True == False
    # return all_sum_plain(g, *args, **kws)
    ...
    def _get_device(self, device_name: str):
        ...
        with tfutil.absolute_name_scope(self.scope + "/Devices"), tflex.device(device_name), tf.control_dependencies(None):
            ...
            if self.use_loss_scaling:
                device.loss_scaling_var = tf.Variable(np.float32(self.loss_scaling_init), trainable = False, name = "loss_scaling_var", use_resource=True)
                ...
                if self.minibatch_multiplier is None:
                    ...
                    with tf.control_dependencies(None):
                        for var in device.grad_clean.keys():
                            device.grad_acc_vars[var] = tf.Variable(tf.zeros(var.shape), trainable = False, name = "grad_acc_var", use_resource=True)
                        device.grad_acc_count = tf.Variable(tf.zeros([]), trainable = False, name = "grad_acc_count", use_resource=True)
...
class SimpleAdam:
    ...
    def apply_gradients(self, grads_and_vars):
        with tf.name_scope(self.name):
            ...
            with tf.control_dependencies(None):
                b1pow_var = tf.Variable(dtype=tf.float32, initial_value=1, trainable=False, use_resource=True)
                b2pow_var = tf.Variable(dtype=tf.float32, initial_value=1, trainable=False, use_resource=True)
            ...
            for grad, var in grads_and_vars:
                with tf.control_dependencies(None):
                    m_var = tf.Variable(dtype=tf.float32, initial_value=tf.zeros_like(var), trainable=False, use_resource=True)
                    v_var = tf.Variable(dtype=tf.float32, initial_value=tf.zeros_like(var), trainable=False, use_resource=True)
